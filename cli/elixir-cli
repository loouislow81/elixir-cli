...
downloadCompleted.promise.then(function() {
  ...
  debug('[elixir-cli] writing video file to ' + videoFileName + '...');
  fs.writeFileSync(videoFileName, data);

  debug('');
  debug('[elixir-cli] converting MP3 to ' + musicFileName);
  /* Convert to an mp3 */
...
convertCompleted.promise.then(function() {
  debug('');
  debug(colors.cyan('[elixir-cli] processing video metadata...'));
  ...
  debug('');
  debug('[elixir-cli] writing mp3 metadata...');
  ffMetadata.write(musicFileName, metadata, function(err) {
    if (err) warning(err, "[elixir-cli] [err] failed to write mp3 metadata.", err);
...
metadataCompleted.promise.then(function(metadata) {
  ...
  debug('[elixir-cli] writing final mp3 file: ' + outputFileName);
  try {
...
function warning(err, msg) {
  console.log('\n' + colors.bold(colors.cyan('WARNING: ')) + colors.cyan(msg));
}
...
function printHeader() {
  console.log(colors.bold(colors.cyan("\nElixir-Cli (v" + packageJson.version + ")\n")));
}
